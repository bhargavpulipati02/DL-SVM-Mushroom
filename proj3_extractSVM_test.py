...
import joblib
import os
# ------------ run using : python proj3_extractSVM_test.py --model trained_cnn_model.keras --svm trained_svm_classifier.joblib --test_csv sample_test_data/mushrooms_test.csv---------
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Feature Extraction + SVM Test")
    parser.add_argument('--model', type=str, default='trained_cnn_model.keras', help='Saved CNN model')
    parser.add_argument('--svm', type=str, default='trained_svm_classifier.joblib', help='Saved SVM model')
    ...
    feature_extractor = tf.keras.Model(inputs=my_model.input, outputs=my_model.layers[-2].output)
    features = feature_extractor.predict(test_images, verbose=0)

    # Load SVM model
    svm_clf = joblib.load(args.svm)

    # Predict and evaluate
    pred_labels = svm_clf.predict(features)
    svm_acc = np.mean(pred_labels == test_labels)
    print('SVM classifier accuracy: Test model, accuracy: {:5.5f}%'.format(100 * svm_acc))
